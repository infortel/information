            ...
            if ((computerId==Constants.getComputerProgramID()) || (Constants.DEBUGGING)) {
                ...
                if (reportOk(qAgent.open(true))) {
                    if (qAgent.next()) {
                        ...
                        out.println("<br>C骴igo no pudo ser encontrado");
                    }
                }
            } else {
                out.println("<br>Error en c骴igo de computadora");
            }
        } catch (Throwable e) {
...
//******************************************************************************
    private void presentIntroduction() {
        ...
        if (pastDueDays>0) {
            out.println("Sus cuentas se encuentran con una mora de "+(pastDueDays/30)+" meses y "+pastDueDays%30+" d铆as<br>");
        } else {
            out.println("Sus cuentas se encuentran al d铆a.<br>");
        }
        
        if (fecha_extension_licencia==null) {
            out.println("No existe ninguna extensi贸n.<br>");
        } else {
        ...
        if (daysActivated>0) {
            if (manualExtensionAuthorized) {
                out.println("Nota: **** Activaci贸n especial autorizada **** <br>");
                out.println("<br>");
            }
            out.println("Activado por = "+daysActivated+" d铆as hasta el "+expires.getString("dd/MM/yyyy")+"<br>");
        } else {
...
//******************************************************************************
    private void presentAccountStatement(String code) {
        try {
            //Present Title
            String name="??? No se ubic贸 el c贸digo ???";
            ...
            out.println(COL1+"MORA(d铆as)"+COL2);
            out.println("</tr>");
